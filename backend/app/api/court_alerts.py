...
from ..services.report_store import (
    REPORTS_DIR,
    delete_report_file,
    load_court_alerts_report,
    save_court_alerts_report,
)
from ..services.resumable_uploads import resolve_trusted_final_path
from ..services.court_alerts import analyze, build_export_workbook, result_from_metrics_dict
from ..utils.file_handler import UPLOAD_DIR

court_alerts_bp = Blueprint("court_alerts", __name__)

CA_ROOT = UPLOAD_DIR / "court_alerts"
    ...
    try:
        _write_job_progress(
            job_dir,
            {
                ...
                "status": "running",
                "progress": 10,
                "message": "Starting Court Alerts analysis…",
            },
        )
        result = analyze(
            court_alerts_path,
        ...
        save_court_alerts_report(job_id, metrics=metrics, created_at=created_at)
...
def _job_snapshot(job_id: str) -> Dict[str, Any] | None:
    ...
    if live:
        if live.get("status") == "running":
            progress = _read_job_progress(CA_ROOT / job_id)
            if progress:
                live = {**live, **progress}
        return live
...
def load_court_alerts_from_disk() -> None:
    ca_dir = REPORTS_DIR / "court_alerts"
    if not ca_dir.is_dir():
        return
    for path in ca_dir.glob("*.json"):
        try:
            job_id = path.stem
            loaded = load_court_alerts_report(job_id)
            if loaded:
                _job_results[job_id] = {
...
@court_alerts_bp.route("/analyze", methods=["POST"])
def court_alerts_analyze():
    if request.is_json:
        data = request.get_json() or {}
        ca_raw = (data.get("court_alerts_path") or "").strip()
        reisift_raw = (data.get("reisift_path") or "").strip()
        ql_raw = (data.get("qualified_leads_path") or data.get("ql_path") or "").strip()
        opps_raw = (data.get("opportunities_path") or "").strip() or None
        txn_raw = (data.get("transactions_path") or "").strip() or None
        if not ca_raw:
            return jsonify({"detail": "court_alerts_path is required"}), 400
        if not reisift_raw:
            return jsonify({"detail": "reisift_path is required"}), 400
        if not ql_raw:
            return jsonify({"detail": "qualified_leads_path is required"}), 400
        try:
            court_alerts_path = str(resolve_trusted_final_path(ca_raw))
        ...
        payload, status = _start_court_alerts_job(
            court_alerts_path,
            reisift_path,
            ql_path,
            opportunities_path,
    ...
    court_alerts = request.files.get("court_alerts_file")
    reisift = request.files.get("reisift_file")
    ql = request.files.get("qualified_leads_file") or request.files.get("ql_file")
    if not court_alerts or not court_alerts.filename:
        return jsonify({"detail": "court_alerts_file is required"}), 400
    ...
    job_dir = CA_ROOT / job_id
    job_dir.mkdir(parents=True, exist_ok=True)
    court_alerts_path = _save_upload(court_alerts, job_dir)
...
@court_alerts_bp.route("/<job_id>/status", methods=["GET"])
def court_alerts_status(job_id: str):
    _sync_job_from_disk(job_id)
...
@court_alerts_bp.route("/<job_id>", methods=["GET"])
def court_alerts_get(job_id: str):
    snap = _job_snapshot(job_id)
    if snap:
...
@court_alerts_bp.route("/<job_id>/export", methods=["GET"])
def court_alerts_export(job_id: str):
    cached = _job_results.get(job_id)
    if not cached:
        loaded = load_court_alerts_report(job_id)
        if loaded:
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f"court_alerts_{job_id}.xlsx",
    )


@court_alerts_bp.route("/<job_id>", methods=["DELETE"])
def court_alerts_delete(job_id: str):
