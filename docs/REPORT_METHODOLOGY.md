...
**Geographic universe:** NY, Nassau and Suffolk counties only. Apply the consolidated exclusions in `backend/app/services/buybox_policy.json`: a present ZIP is decisive; use excluded-city names only if ZIP is absent. Malformed nonblank ZIPs and missing county/state are excluded with reason counts. Scores do not determine admission. Rows outside this geography are dropped before collapse/enrichment/KPIs. Optional property details are streamed after collapse: returned APN/county must validate against original sold evidence. Supported snapshot rules screen property type/units, age, estimated value, living and lot area; excluded and unresolved properties are audited separately and never enter screened KPIs. Ownership duration, LTV and non-seller/religious-owner exclusion remain unevaluated. Without details the report remains geography-only. Matched historical sellers receive name-based Trust/Company/Individual categories; unmatched or ambiguous events remain Unclassified, never replaced with current owners. **Full contract:** [BUYBOX.md](BUYBOX.md). Address keys are rebuilt from property parts with a normalized ZIP, not the scrape's `address_key`.

**Required inputs:** Sold CSV + REISift export + Salesforce Total Qualified Leads. Recommended ZIP bundles additionally require `sold_property_details.jsonl`; canonical bundle names are in [BUYBOX.md](BUYBOX.md#upload-bundle). Individual-file uploads can omit details for geography-only analysis. Opportunities optional (Opp stage). **Salesforce Transaction Pipeline** optional (`Closed Date` → Closed; `Date Contract Signed` / accepted offer → Opportunity / under contract). This is the single sold-properties product report; the former REISift `in_sold_properties_full` cohort report was removed.

**Grain:** One **property across the entire uploaded report**, anchored to its earliest observed sold month. Identity prefers `dataflik_id`, falling back to the normalized property address; a missing ID joins a known ID at that address only if unambiguous. Distinct IDs are not merged merely because an address matches. `transaction_count` counts distinct nonblank transaction IDs (otherwise distinct month/buyer/amount observations), including later sales, and `last_sold_month` retains the latest observed month. Later transactions do not change earliest-month investor/in-list flags or pipeline cutoffs. Because the source provides monthly periods rather than reliable within-month ordering, flags combine only earliest-month observations, distinct buyers are shown together, and conflicting amounts are left blank rather than choosing a price arbitrarily. Monthly rollups assign each property only to its first observed month and sum to the report total. Saved reports retain their original grain; rerun to apply this policy. `sold_rows_ingested` counts retained source rows before deduplication.

**Segments:** Investor (`investor`), In Our List (`in_my_records`), Both, Neither, based on the earliest observed sold month. Rollups by first sold month and segment. Headline Marketed → Closed cards and the Pipeline Funnel are cumulative “reached at least” counts; percentages use all admitted distinct properties as the denominator. Seller ownership classification is not included: the supplied reports do not verify seller identity before the sale.

**Canonical pipeline clocks** (shared helpers in `backend/app/services/sold_properties.py`, after earliest-month property collapse):
