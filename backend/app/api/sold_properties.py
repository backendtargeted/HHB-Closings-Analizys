"""
Gate 6 — Sold properties (pipeline depth before external sale) API.
"""
...
from ..services.report_store import (
    REPORTS_DIR,
    delete_report_file,
    load_sold_properties_report,
    save_sold_properties_report,
)
from ..services.resumable_uploads import resolve_trusted_final_path
from ..services.sold_properties import analyze, build_export_workbook, result_from_metrics_dict
from ..utils.file_handler import UPLOAD_DIR

sold_properties_bp = Blueprint("sold_properties", __name__)

SP_ROOT = UPLOAD_DIR / "sold_properties"
    ...
    try:
        _write_job_progress(
            job_dir,
            {
                ...
                "status": "running",
                "progress": 10,
                "message": "Starting sold-properties analysis…",
            },
        )
        result = analyze(
            reisift_path,
        ...
        save_sold_properties_report(job_id, metrics=metrics, created_at=created_at)
...
def _job_snapshot(job_id: str) -> Dict[str, Any] | None:
    ...
    if live:
        if live.get("status") == "running":
            progress = _read_job_progress(SP_ROOT / job_id)
            if progress:
                live = {**live, **progress}
        return live
...
def load_sold_properties_from_disk() -> None:
    sp_dir = REPORTS_DIR / "sold_properties"
    if not sp_dir.is_dir():
        return
    for path in sp_dir.glob("*.json"):
        try:
            job_id = path.stem
            loaded = load_sold_properties_report(job_id)
            if loaded:
                _job_results[job_id] = {
...
def _run_sold_properties_job(
    ...
    proc = multiprocessing.Process(
        ...
        args=(job_id, reisift_path, ql_path, opportunities_path, str(job_dir)),
        daemon=True,
    )
    proc.start()
    threading.Thread(
...
def _start_sold_properties_job(
    reisift_path: str,
    ql_path: str,
    opportunities_path: Optional[str],
    ...
    thread = threading.Thread(
        target=_run_sold_properties_job,
        args=(job_id, reisift_path, ql_path, opportunities_path),
        daemon=True,
    )
    thread.start()
    return (
...
@sold_properties_bp.route("/analyze", methods=["POST"])
def sold_properties_analyze():
    if request.is_json:
        ...
        payload, status = _start_sold_properties_job(
            reisift_path, ql_path, opportunities_path
        )
...
@sold_properties_bp.route("/<job_id>/status", methods=["GET"])
def sold_properties_status(job_id: str):
    _sync_job_from_disk(job_id)
...
@sold_properties_bp.route("/<job_id>", methods=["GET"])
def sold_properties_get(job_id: str):
    snap = _job_snapshot(job_id)
    if snap:
...
@sold_properties_bp.route("/<job_id>/export", methods=["GET"])
def sold_properties_export(job_id: str):
    cached = _job_results.get(job_id)
    if not cached:
        loaded = load_sold_properties_report(job_id)
        if loaded:
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f"sold_properties_{job_id}.xlsx",
    )


@sold_properties_bp.route("/<job_id>", methods=["DELETE"])
def sold_properties_delete(job_id: str):
