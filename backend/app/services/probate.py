...
QL_CAMPAIGN_CANDIDATES = ["Campaign", "Campaign Name", "Primary Campaign Source"]
...
def _build_match_index(
    ...
    campaign_candidates: Optional[List[str]] = None,
) -> MatchIndex:
    ...
    campaign_col = find_column_name(df, campaign_candidates) if campaign_candidates else None
    by_full: Dict[str, List[int]] = {}
    by_no_state: Dict[str, List[int]] = {}
    by_street_zip: Dict[str, List[int]] = {}
    by_phone: Dict[str, List[int]] = {}
...
def _row_kwargs(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out["prospect_source"] = str(out.get("prospect_source") or "")
    out["prospect_source_label"] = str(out.get("prospect_source_label") or "")
    out["ql_campaign"] = str(out.get("ql_campaign") or "")
