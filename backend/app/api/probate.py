"""
Gate 5 — Probate lifecycle report API.
"""
...
from ..services.probate import analyze, build_export_workbook, result_from_metrics_dict
from ..utils.file_handler import UPLOAD_DIR

probate_bp = Blueprint("probate", __name__)

PB_ROOT = UPLOAD_DIR / "probate"
    ...
    try:
        _write_job_progress(
            job_dir,
            {
                ...
                "status": "running",
                "progress": 10,
                "message": "Starting probate analysis…",
            },
        )
        result = analyze(
            reisift_path,
        ...
        save_probate_report(job_id, metrics=metrics, created_at=created_at)
...
def _job_snapshot(job_id: str) -> Dict[str, Any] | None:
    ...
    if live:
        if live.get("status") == "running":
            progress = _read_job_progress(PB_ROOT / job_id)
            if progress:
                live = {**live, **progress}
        return live
...
def load_probate_from_disk() -> None:
    pb_dir = REPORTS_DIR / "probate"
    if not pb_dir.is_dir():
        return
    for path in pb_dir.glob("*.json"):
        try:
            job_id = path.stem
...
def _start_probate_job(
    ...
    thread = threading.Thread(
        ...
        args=(job_id, reisift_path, ql_path, opportunities_path, transactions_path),
        daemon=True,
    )
    thread.start()
    return (
...
@probate_bp.route("/analyze", methods=["POST"])
def probate_analyze():
    if request.is_json:
        ...
        payload, status = _start_probate_job(
            reisift_path, ql_path, opportunities_path, transactions_path
        )
...
@probate_bp.route("/<job_id>/status", methods=["GET"])
def probate_status(job_id: str):
    _sync_job_from_disk(job_id)
...
@probate_bp.route("/<job_id>", methods=["GET"])
def probate_get(job_id: str):
    snap = _job_snapshot(job_id)
    if snap:
...
@probate_bp.route("/<job_id>/export", methods=["GET"])
def probate_export(job_id: str):
    cached = _job_results.get(job_id)
    if not cached:
        loaded = load_probate_report(job_id)
        if loaded:
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f"probate_{job_id}.xlsx",
    )


@probate_bp.route("/<job_id>", methods=["DELETE"])
def probate_delete(job_id: str):
