...
interface CourtAlertsResultsProps {
  result: CourtAlertsCompletedResponse;
  onNewRun: () => void;
  onExport: () => void;
  exporting: boolean;
}
...
const CourtAlertsResults = ({
  result,
  onNewRun,
  onExport,
  exporting,
}: CourtAlertsResultsProps) => {
  ...
  const handleShare = async () => {
    setShareStatus(null);
    const mode = await copyReportShareUrl(result.job_id, 'court_alerts');
  ...
  const filteredRows: CourtAlertsRow[] = m.rows.filter((row) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-stone-600 mt-1">
            ...
            {' · '}
            {m.inputs.ca_universe.toLocaleString()} Court Alerts properties
            {' · '}
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            onClick={onExport}
            disabled={exporting}
            className="px-4 py-2 rounded-lg border border-sky-700 text-sky-900 text-sm font-medium hover:bg-sky-50 disabled:opacity-50"
          >
          ...
          <button
            type="button"
            onClick={handleShare}
            className="px-4 py-2 rounded-lg border border-stone-300 text-sky-900 text-sm font-medium hover:bg-stone-50"
          >
            Copy Link
          </button>
          <button
            type="button"
            onClick={onNewRun}
            className="px-4 py-2 rounded-lg bg-sky-800 text-white text-sm font-medium hover:bg-sky-900"
          >
            New run
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Stat label="Mean months to CRM" value={fmt(m.lag.mean_months_ca_to_prospect)} />
        <Stat label="Median CA to CRM" value={fmt(m.lag.by_source?.ca?.median)} />
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-5">
        ...
        <VerticalBars
          data={funnelChart}
          series={[{ key: 'count', name: 'Properties', color: '#0369a1' }]}
        />
      </section>
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-5">
        ...
        <p className="text-sm text-stone-600 mt-1">
          QL credit is this first list. Court Alerts created_on month vs List Purchased 8020 on the
          matched REISift address.
        </p>
        <VerticalBars
          ...
          series={[
            ...
            { key: 'prospects', name: 'Prospects', color: '#0284c7' },
          ]}
        />
      </section>
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-5">
        ...
        {lagChart.length > 0 ? (
          <VerticalBars
            ...
            grouped
            series={[
              { key: 'ca', name: 'Court Alerts', color: '#0369a1' },
      ...
      <CompactTable
        ...
        columns={['Bucket', 'Court Alerts', '8020', 'Same month', 'Total']}
        rows={m.lag_buckets
          .filter((r) => (r.count ?? 0) > 0)
          .map((r) => [
            r.bucket ?? '',
      ...
      <section className="rounded-xl border border-stone-200 bg-white p-5">
        <h3 className="text-lg font-semibold text-stone-900">Listed by Court Alerts month</h3>
        <p className="text-sm text-stone-600 mt-1">
          Drop size that month (created_on month start). Recent months have had less time to become
          Prospects.
        </p>
        {monthChart.length > 0 ? (
          <VerticalBars
      ...
      <CompactTable
        ...
        rows={m.cohorts.map((r) => [
          ...
          `${r.prospect_rate_pct ?? 0}%`,
          r.txns ?? 0,
          fmt(r.mean_months_ca_to_prospect),
        ])}
      />

      <CompactTable
        title="In CRM before first list"
        subtitle="Address or phone matched a Qualified Lead, but Create Date is before the first Court Alerts/8020 list month. Not a conversion from this list. Not source credit."
        columns={['Address', 'County', 'Court Alerts', '8020', 'CRM date', 'Match', 'Campaign']}
      ...
      <CompactTable
        ...
        subtitle="From the Transactions pipeline only, after address match."
          ...
          <table className="min-w-full text-xs">
            <thead>
              <tr className="border-b text-left text-stone-500">
                ...
                <th className="py-2 pr-3">Court Alerts</th>
            ...
            <tbody>
              {filteredRows.map((r) => (
                <tr key={r.address_key + r.ca_month + r.address} className="border-b border-stone-100">
                  <td className="py-2 pr-3 whitespace-nowrap">{r.address}</td>
                  <td className="py-2 pr-3">{r.county}</td>
                  <td className="py-2 pr-3">{r.ca_month}</td>
...
export default CourtAlertsResults;
