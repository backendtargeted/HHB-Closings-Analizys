...
assert result.property_rows == 4790
assert len({r.dataflik_id for r in result.rows}) == 4790
...
(out / "verified_report_metrics.json").write_text(json.dumps(metrics, indent=2), encoding="utf-8")
summary = {"inputs": metrics["inputs"], "multi_transaction_properties": sum(r.transaction_count > 1 for r in result.rows),
    "by_first_sold_month": metrics["by_sold_month"], "lost": metrics["lost"], "match": metrics["match"]}
(out / "verified_report_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
